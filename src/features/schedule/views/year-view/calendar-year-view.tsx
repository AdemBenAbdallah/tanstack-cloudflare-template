import { addDays, format, getYear, isSameDay, isSameMonth } from "date-fns";
...
import { getCalendarCells } from "@/features/schedule/helpers";
...
export function CalendarYearView({ singleDayEvents, multiDayEvents }: IProps) {
  ...
  const fmt = (date: Date, fmtStr: string) =>
    dateFnsLocale
      ? format(date, fmtStr, { locale: dateFnsLocale })
      : format(date, fmtStr);
  const months = Array.from({ length: 12 }, (_, i) => ({
    index: i,
    name: fmt(new Date(currentYear, i, 1), "MMMM"),
  }));
  const weekDays = Array.from({ length: 7 }, (_, i) =>
    fmt(addDays(new Date(2024, 0, 7), i), "EE"),
  );
  ...
  return (
    <div className="flex flex-col h-full  overflow-y-auto p-4  sm:p-6">
      ...
      >
        {months.map(({ index: monthIndex, name: month }) => {
