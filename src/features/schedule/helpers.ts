...
import type { ICalendarCell, IEvent } from "@/features/schedule/interfaces";
import type { TCalendarView, TEventColor } from "@/features/schedule/types";

const FORMAT_STRING = "MMM d, yyyy";
export const HOUR_HEIGHT_PX = 96;

type DateFnsLocale = import("date-fns").Locale;

function fmt(date: Date, fmtStr: string, locale?: DateFnsLocale): string {
  return locale ? format(date, fmtStr, { locale }) : format(date, fmtStr);
}
...
export function rangeText(
  ...
  switch (view) {
    ...
    case "week":
      ...
      break;
    case "day":
      return fmt(date, FORMAT_STRING, locale);
    case "year":
  ...
  return `${fmt(start, FORMAT_STRING, locale)} - ${fmt(end, FORMAT_STRING, locale)}`;
}
...
export function formatTime(
  ...
  const fmtStr = use24HourFormat ? "HH:mm" : "h:mm a";
  return locale
    ? format(parsedDate, fmtStr, { locale })
    : format(parsedDate, fmtStr);
}
